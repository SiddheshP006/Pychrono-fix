...
class PID_controller:
  def __init__(self, uav_params):
    ...
    self.G_acc = uav_params.G_acc
    self.mass_total_estimated = uav_params.mass_total_estimated
    self.I_matrix_estimated = uav_params.I_matrix_estimated
    self.A_phi_ref = uav_params.A_phi_ref
    self.B_phi_ref = uav_params.B_phi_ref
    self.C_phi_ref = uav_params.C_phi_ref
    self.A_theta_ref = uav_params.A_theta_ref
    self.B_theta_ref = uav_params.B_theta_ref
    self.C_theta_ref = uav_params.C_theta_ref
    
  def set_gains(self, params):
      #self.dy = [0] * params.number_of_states
      self.dy = np.zeros((params.number_of_states, 1))
      self.KP_tran = params.KP_tran
      self.KD_tran = params.KD_tran
      self.KI_tran = params.KI_tran
      self.KP_rot = params.KP_rot
      self.KD_rot = params.KD_rot
      self.KI_rot = params.KI_rot
      print("from pid")
      print(self.KI_rot)
